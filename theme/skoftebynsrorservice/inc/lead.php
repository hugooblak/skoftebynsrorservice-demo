 ...
 * or photo. They call sk_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Skoftebyns rörservice.
 *
 ...
 * @return string
 */
function sk_sample_note() {
	return sk_lead( 'ar_riktig' ) ? '' : ' <span class="sk-sample">(platshållare)</span>';
}
 ...
 * @return string File name for sk_lead_img().
 */
function sk_lead_bild( $i ) {
	$bilder = (array) sk_lead( 'bilder', array() );
 ...
 * @return string
 */
function sk_ring_label() {
	$namn = sk_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, sk_lead( 'telefon' ) ) : 'Ring ' . sk_lead( 'telefon' );
}
