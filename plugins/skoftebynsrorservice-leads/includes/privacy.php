...
add_filter(
	...
	function ( $exporters ) {
		$exporters['skoftebynsrorservice-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'skoftebynsrorservice-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'skoftebynsrorservice-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'skoftebynsrorservice-leads',
						'group_label' => __( 'Quote requests', 'skoftebynsrorservice-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['skoftebynsrorservice-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'skoftebynsrorservice-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
