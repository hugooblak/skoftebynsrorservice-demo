...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'skoftebynsrorservice-leads' ),
					'singular_name' => __( 'Lead', 'skoftebynsrorservice-leads' ),
					'edit_item'     => __( 'Förfrågan', 'skoftebynsrorservice-leads' ),
					'search_items'  => __( 'Search leads', 'skoftebynsrorservice-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'skoftebynsrorservice-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'skoftebynsrorservice-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'skoftebynsrorservice-leads' ),
			'npl_phone'    => __( 'Phone', 'skoftebynsrorservice-leads' ),
			'npl_service'  => __( 'Needs', 'skoftebynsrorservice-leads' ),
			'npl_timeline' => __( 'When', 'skoftebynsrorservice-leads' ),
			'npl_zip'      => __( 'ZIP', 'skoftebynsrorservice-leads' ),
			'npl_source'   => __( 'Source', 'skoftebynsrorservice-leads' ),
			'npl_status'   => __( 'Status', 'skoftebynsrorservice-leads' ),
			'date'         => __( 'Received', 'skoftebynsrorservice-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'skoftebynsrorservice-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'skoftebynsrorservice-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'skoftebynsrorservice-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'skoftebynsrorservice-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'skoftebynsrorservice-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'skoftebynsrorservice-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'skoftebynsrorservice-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'skoftebynsrorservice-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'skoftebynsrorservice-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'skoftebynsrorservice-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'skoftebynsrorservice-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'skoftebynsrorservice-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
